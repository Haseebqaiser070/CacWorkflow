...
export default function SyllabusFinal() {
  ...
  const getstuff = async () => {
    const res = await axios.get(
      `http://localhost:4000/CDF/showOne/${Program}/${Code}`
    );
    setCDF(res.data);
    var sooo = [];
  ...
  const getCat = async () => {
    try {
      const response = await axios.get(
        `http://localhost:4000/ProgramCourses/showcode/${Program}/${Code}`
      );
  ...
  const getContent = async () => {
    const response = await axios.get(`http://localhost:4000/Syllabus/${id}`);
